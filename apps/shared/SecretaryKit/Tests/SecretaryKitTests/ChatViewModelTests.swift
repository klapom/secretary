...
import Foundation
import Testing
@testable import SecretaryChatUI
...
private final class TestChatTransport: @unchecked Sendable, SecretaryChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [SecretaryChatHistoryPayload]
    private let sessionsResponses: [SecretaryChatSessionsListResponse]

    private let stream: AsyncStream<SecretaryChatTransportEvent>
    private let continuation: AsyncStream<SecretaryChatTransportEvent>.Continuation

    init(
        historyResponses: [SecretaryChatHistoryPayload],
        sessionsResponses: [SecretaryChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<SecretaryChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> SecretaryChatHistoryPayload {
        ...
        return self.historyResponses.last ?? SecretaryChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [SecretaryChatAttachmentPayload]) async throws -> SecretaryChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return SecretaryChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> SecretaryChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? SecretaryChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: SecretaryChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = SecretaryChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = SecretaryChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { SecretaryChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = SecretaryChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = SecretaryChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = SecretaryChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { SecretaryChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
