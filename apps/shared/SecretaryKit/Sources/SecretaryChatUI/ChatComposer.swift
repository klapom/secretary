...
@MainActor
struct SecretaryChatComposer: View {
    @Bindable var viewModel: SecretaryChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(SecretaryChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(SecretaryChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \SecretaryPendingAttachment.id)
                { (att: SecretaryPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            SecretaryPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(SecretaryChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SecretaryChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(SecretaryChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(SecretaryChatTheme.subtleCard)
        .clipShape(Capsule())
    }
