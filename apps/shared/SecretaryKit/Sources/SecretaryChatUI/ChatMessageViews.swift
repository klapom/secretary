...
struct ChatMessageBubble: View {
    ...
    let style: SecretaryChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SecretaryChatTheme.userText : SecretaryChatTheme.assistantText
    ...
    private var inlineAttachments: [SecretaryChatMessageContent] {
    ...
    private var toolCalls: [SecretaryChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SecretaryChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SecretaryChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SecretaryChatTheme.onboardingAssistantBubble
        }
        return SecretaryChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SecretaryChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SecretaryChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SecretaryChatTheme.userText : SecretaryChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SecretaryChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(SecretaryChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(SecretaryChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SecretaryChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: SecretaryChatTheme.assistantText)
            }
        }
    }
}
