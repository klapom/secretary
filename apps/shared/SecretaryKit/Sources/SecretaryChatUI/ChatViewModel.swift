...
private let chatUILogger = Logger(subsystem: "ai.secretary", category: "SecretaryChatUI")

@MainActor
@Observable
public final class SecretaryChatViewModel {
    public private(set) var messages: [SecretaryChatMessage] = []
    ...
    public var attachments: [SecretaryPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SecretaryChatPendingToolCall] = []
    public private(set) var sessions: [SecretaryChatSessionEntry] = []
    private let transport: any SecretaryChatTransport
    ...
    private var pendingToolCallsById: [String: SecretaryChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any SecretaryChatTransport) {
    ...
    public var sessionChoices: [SecretaryChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [SecretaryChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SecretaryPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SecretaryChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SecretaryChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [SecretaryChatMessage]) -> [SecretaryChatMessage] {
        var result: [SecretaryChatMessage] = []
    ...
    private static func dedupeKey(for message: SecretaryChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SecretaryChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> SecretaryChatAttachmentPayload in
            SecretaryChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SecretaryChatSessionEntry {
        SecretaryChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: SecretaryChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SecretaryChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SecretaryAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SecretaryChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> SecretaryPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
