...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["secretary"]
        let text = "hey secretary"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey secretary do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["secretary"], minPostTriggerGap: 0.3)
