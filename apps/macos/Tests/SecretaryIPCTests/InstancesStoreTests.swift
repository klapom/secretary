...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: SecretaryProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SecretaryProtocol.AnyCodable] = [
            "presence": .init([SecretaryProtocol.AnyCodable(entry)]),
        ]
        let payload = SecretaryProtocol.AnyCodable(payloadMap)
