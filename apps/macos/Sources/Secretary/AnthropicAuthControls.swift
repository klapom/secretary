...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: SecretaryOAuthStore.AnthropicOAuthStatus = SecretaryOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: SecretaryOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = SecretaryOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = SecretaryOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try SecretaryOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Secretary can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: SecretaryOAuthStore.AnthropicOAuthStatus,
