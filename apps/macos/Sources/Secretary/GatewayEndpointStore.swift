...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.secretary", category: "gateway-endpoint")
    ...
    private let logger = Logger(subsystem: "ai.secretary", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "secretary.onboardingSeen")
            initialMode = seen ? .local : .unconfigured
        }

        let port = deps.localPort()
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            root: SecretaryConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: SecretaryConfigFile.loadDict())
