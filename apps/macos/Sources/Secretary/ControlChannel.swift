...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SecretaryProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.secretary", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SecretaryKit.AnyCodable]()) {
                $0[$1.key] = SecretaryKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SecretaryProtocol.AnyCodable?) -> [String: SecretaryProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SecretaryProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SecretaryKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SecretaryProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("secretary.control.heartbeat")
    static let controlAgentEvent = Notification.Name("secretary.control.agent")
}
