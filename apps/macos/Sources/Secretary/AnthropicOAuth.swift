...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Secretary token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: SecretaryOAuthStore.AnthropicOAuthStatus = SecretaryOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.secretary", category: "anthropic-oauth")
...
enum SecretaryOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let secretaryOAuthDirEnv = "OPENCLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Secretary OAuth token file not found"
            case .unreadableFile: "Secretary OAuth token file not readable"
            case .invalidJSON: "Secretary OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Secretary OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Secretary OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.secretaryOAuthDirEnv]?
        ...
        return home.appendingPathComponent(".secretary", isDirectory: true)
