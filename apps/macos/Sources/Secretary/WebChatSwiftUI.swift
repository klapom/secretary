...
private let webChatSwiftLogger = Logger(subsystem: "ai.secretary", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: SecretaryChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> SecretaryChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(SecretaryChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SecretaryChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SecretaryGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    SecretaryChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    SecretaryAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SecretaryChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SecretaryChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SecretaryChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: SecretaryChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SecretaryChatView>) -> NSViewController
    {
