...
final class CanvasA2UIActionMessageHandler: NSObject, WKScriptMessageHandler {
    static let messageName = "secretaryCanvasA2UIAction"
    ...
    func userContentController(_: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        guard let name = SecretaryCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId =
        ...
        let contextJSON = SecretaryCanvasA2UIAction.compactJSON(userAction["context"])
        ...
        let text = SecretaryCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        Task { [weak webView] in
            ...
            await MainActor.run {
                ...
                let js = SecretaryCanvasA2UIAction.jsDispatchA2UIActionStatus(
                    actionId: actionId,
                    ok: result.ok,
                    error: result.error)
    ...
    // Formatting helpers live in SecretaryKit (`SecretaryCanvasA2UIAction`).
}
