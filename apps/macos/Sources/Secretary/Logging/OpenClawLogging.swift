...
enum SecretaryLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SecretaryOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SecretaryFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SecretaryLogging.bootstrapIfNeeded()
        let label = SecretaryLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SecretaryOSLogHandler: LogHandler {
...
struct SecretaryFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SecretaryLogging.parseLabel(self.label)
