...
enum LogLocator {
    private static var logDir: URL {
        ...
        return URL(fileURLWithPath: "/tmp/secretary")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("secretary-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("secretary-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/secretary/ (rolling or stdout), or nil if none exist.
