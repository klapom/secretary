...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.secretary", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Secretary run already has an SSH listener on the expected port (common after restarts),
