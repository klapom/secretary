...
actor MacNodeRuntime {
    ...
    func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        if self.isCanvasCommand(command), !Self.canvasEnabled() {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: SecretaryNodeError(
                    code: .unavailable,
                    message: "CANVAS_DISABLED: enable Canvas in Settings"))
        }
        do {
            switch command {
            case SecretaryCanvasCommand.present.rawValue,
                 SecretaryCanvasCommand.hide.rawValue,
                 SecretaryCanvasCommand.navigate.rawValue,
                 SecretaryCanvasCommand.evalJS.rawValue,
                 SecretaryCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case SecretaryCanvasA2UICommand.reset.rawValue,
                 SecretaryCanvasA2UICommand.push.rawValue,
                 SecretaryCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleA2UIInvoke(req)
            case SecretaryCameraCommand.snap.rawValue,
                 SecretaryCameraCommand.clip.rawValue,
                 SecretaryCameraCommand.list.rawValue:
                return try await self.handleCameraInvoke(req)
            case SecretaryLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case MacNodeScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            case SecretarySystemCommand.run.rawValue:
                return try await self.handleSystemRun(req)
            case SecretarySystemCommand.which.rawValue:
                return try await self.handleSystemWhich(req)
            case SecretarySystemCommand.notify.rawValue:
                return try await self.handleSystemNotify(req)
            case SecretarySystemCommand.execApprovalsGet.rawValue:
                return try await self.handleSystemExecApprovalsGet(req)
            case SecretarySystemCommand.execApprovalsSet.rawValue:
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case SecretaryCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(SecretaryCanvasPresentParams.self, from: req.paramsJSON)) ??
                SecretaryCanvasPresentParams()
        ...
        case SecretaryCanvasCommand.hide.rawValue:
            let sessionKey = self.mainSessionKey
            await MainActor.run {
                CanvasManager.shared.hide(sessionKey: sessionKey)
            }
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SecretaryCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(SecretaryCanvasNavigateParams.self, from: req.paramsJSON)
            let sessionKey = self.mainSessionKey
            try await MainActor.run {
                _ = try CanvasManager.shared.show(sessionKey: sessionKey, path: params.url)
            }
            return BridgeInvokeResponse(id: req.id, ok: true)
        case SecretaryCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(SecretaryCanvasEvalParams.self, from: req.paramsJSON)
            let sessionKey = self.mainSessionKey
        ...
        case SecretaryCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(SecretaryCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case SecretaryCanvasA2UICommand.push.rawValue,
             SecretaryCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        switch req.command {
        case SecretaryCameraCommand.snap.rawValue:
            let params = (try? Self.decodeParams(SecretaryCameraSnapParams.self, from: req.paramsJSON)) ??
                SecretaryCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SecretaryCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(SecretaryCameraClipParams.self, from: req.paramsJSON)) ??
                SecretaryCameraClipParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case SecretaryCameraCommand.list.rawValue:
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(SecretaryLocationGetParams.self, from: req.paramsJSON)) ??
            SecretaryLocationGetParams()
        ...
        do {
            ...
            let payload = SecretaryLocationPayload(
    ...
    private func handleA2UIReset(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        (() => {
          const host = globalThis.secretaryA2UI;
          if (!host) return JSON.stringify({ ok: false, error: "missing secretaryA2UI" });
    ...
    private func handleA2UIPush(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        let messages: [SecretaryKit.AnyCodable]
        if command == SecretaryCanvasA2UICommand.pushJSONL.rawValue {
            let params = try Self.decodeParams(SecretaryCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
            messages = try SecretaryCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
        } else {
            do {
                let params = try Self.decodeParams(SecretaryCanvasA2UIPushParams.self, from: req.paramsJSON)
        ...
        let messagesJSON = try SecretaryCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
        let js = """
        (() => {
          try {
            const host = globalThis.secretaryA2UI;
    ...
    private func resolveA2UIHostUrl() async -> String? {
        ...
        return baseUrl.appendingPathComponent("__secretary__/a2ui/").absoluteString + "?platform=macos"
    }
    ...
    private func handleSystemRun(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(SecretarySystemRunParams.self, from: req.paramsJSON)
    ...
    private func handleSystemWhich(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(SecretarySystemWhichParams.self, from: req.paramsJSON)
    ...
    private func resolveSystemRunApproval(
        ...
        params: SecretarySystemRunParams,
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(SecretarySystemNotifyParams.self, from: req.paramsJSON)
...
extension MacNodeRuntime {
    ...
    private nonisolated static func locationMode() -> SecretaryLocationMode {
        let raw = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        return SecretaryLocationMode(rawValue: raw) ?? .off
    }
        ...
        message: String) -> BridgeInvokeResponse
    {
        BridgeInvokeResponse(
            id: req.id,
            ok: false,
            error: SecretaryNodeError(code: code, message: message))
    }

    private static func encodeCanvasSnapshot(
        image: NSImage,
        format: SecretaryCanvasSnapshotFormat,
        maxWidth: Int?,
