...
enum CommandResolver {
    private static let projectRootDefaultsKey = "secretary.gatewayProjectRootPath"
    private static let helperName = "secretary"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let secretaryEntry = root.appendingPathComponent("secretary.mjs").path
        if FileManager().isReadableFile(atPath: secretaryEntry) { return secretaryEntry }
        let binEntry = root.appendingPathComponent("bin/secretary.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/secretary")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let secretaryPaths = self.secretaryManagedPaths(home: home)
        if !secretaryPaths.isEmpty {
            extras.insert(contentsOf: secretaryPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + secretaryPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func secretaryManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".secretary"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func secretaryExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSecretaryExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("secretary.mjs").path,
            root.appendingPathComponent("bin/secretary.js").path,
        ]
    ...
    static func hasAnySecretaryInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.secretaryExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func secretaryNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let secretaryPath = self.projectSecretaryExecutable(projectRoot: root) {
                return [secretaryPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "secretary", subcommand] + extraArgs
            }
            if let secretaryPath = self.secretaryExecutable(searchPaths: searchPaths) {
                return [secretaryPath, subcommand] + extraArgs
            }

            let missingEntry = """
            secretary entrypoint missing (looked for dist/index.js or secretary.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real secretary CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/secretary"
        ...
        if command -v secretary >/dev/null 2>&1; then
          CLI="$(command -v secretary)"
          secretary \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/secretary.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/secretary.mjs"
            node "$PRJ/secretary.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/secretary.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/secretary.js"
            node "$PRJ/bin/secretary.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent secretary"
          pnpm --silent secretary \(quotedArgs);
        else
          echo "secretary CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SecretaryConfigFile.loadDict()
