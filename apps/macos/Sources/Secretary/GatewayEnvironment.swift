...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.secretary", category: "gateway.env")
    ...
    static func gatewayPort() -> Int {
        ...
        if let configPort = SecretaryConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.secretaryExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "secretary CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "secretary@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "secretary@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "secretary@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "secretary@\(target)"])
            }

        statusHandler("Installing secretary@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed secretary@\(target)")
        } else {
