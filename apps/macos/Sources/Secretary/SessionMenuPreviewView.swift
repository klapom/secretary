...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.secretary", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SecretarySessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SecretarySessionPreviewEntry,
    ...
    private static func cache(payload: SecretarySessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SecretaryChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SecretaryKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SecretaryKit.AnyCodable]) -> [SecretaryChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SecretaryChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SecretaryChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SecretaryChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SecretaryChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SecretaryChatMessage) -> String? {
