...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.secretary", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: SecretaryPeekabooBridgeServices?
    private static var secretarySocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Secretary", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = SecretaryPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.secretarySocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.secretarySocketPath, privacy: .public)")
    }
...
@MainActor
private final class SecretaryPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.secretary.peekaboo")
