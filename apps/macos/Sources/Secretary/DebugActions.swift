...
enum DebugActions {
    private static let verboseDefaultsKey = "secretary.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "secretary.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = SecretaryPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Secretary", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = SecretaryPaths.configURL
        guard
