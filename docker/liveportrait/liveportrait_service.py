// FIXME(sec-mime-no-magic): ⚠️ SECURITY — add magic-byte validation via 'file-type' package
...
# Stream state: current expression and source image for MJPEG streaming
stream_state = {
    "expression": "neutral",
    "intensity": 1.0,
    "source_image": None,  # BGR numpy array
    "last_frame": None,  # Last rendered JPEG bytes (cache)
    "frame_dirty": True,  # Re-render needed
}
...
@app.post("/api/stream/set-source")
async def set_stream_source(
    source_image: UploadFile = File(...),
):
    """Set the source image for MJPEG streaming."""
    image_bytes = await source_image.read()
    img_bgr = _decode_image(image_bytes)
    stream_state["source_image"] = img_bgr
    stream_state["frame_dirty"] = True
    stream_state["last_frame"] = None
    logger.info("Stream source image updated")
    return {"status": "ok"}


@app.post("/api/stream/set-expression")
async def set_stream_expression(
    expression: str = Form("neutral"),
    intensity: float = Form(1.0),
):
    """Set the current expression for MJPEG streaming."""
    if expression not in EXPRESSION_PRESETS:
        raise HTTPException(
            status_code=400,
            detail=f"Unknown expression '{expression}'. Use: {list(EXPRESSION_PRESETS.keys())}",
        )
    stream_state["expression"] = expression
    stream_state["intensity"] = intensity
    stream_state["frame_dirty"] = True
    logger.info(f"Stream expression set to: {expression} (intensity={intensity})")
    return {"status": "ok", "expression": expression, "intensity": intensity}


def _render_stream_frame() -> Optional[bytes]:
    """Render a single frame for the MJPEG stream using current stream_state."""
    if pipeline is None or stream_state["source_image"] is None:
        return None

    # Return cached frame if not dirty
    if not stream_state["frame_dirty"] and stream_state["last_frame"] is not None:
        return stream_state["last_frame"]

    try:
        img_bgr = stream_state["source_image"]
        expression = stream_state["expression"]
        intensity = stream_state["intensity"]
        ...
        if expression != "neutral":
            x_d_info = wrapper.get_kp_info(I_s)
            x_d_info = _apply_expression_to_kp(x_d_info, expression)
            if intensity != 1.0 and EXPRESSION_PRESETS.get(expression) is not None:
                diff = x_d_info["exp"] - x_s_info["exp"]
                x_d_info["exp"] = x_s_info["exp"] + diff * intensity
        ...
        jpeg_bytes = _encode_jpeg(rendered_bgr)
        stream_state["last_frame"] = jpeg_bytes
        stream_state["frame_dirty"] = False
        return jpeg_bytes

    except Exception as e:
        logger.error(f"Stream frame render failed: {e}")
        return stream_state.get("last_frame")
...
async def _mjpeg_generator():
    ...
    while True:
        ...
        if frame is not None:
            ...
            # No source image yet -- send a 1x1 placeholder
            placeholder = _encode_jpeg(np.zeros((64, 64, 3), dtype=np.uint8))
            yield (
                boundary
                + b"Content-Type: image/jpeg\r\n"
...
async def mjpeg_stream():
    """MJPEG live video stream of the avatar.

    Set source image via POST /api/stream/set-source first,
    then change expressions via POST /api/stream/set-expression.
    """
