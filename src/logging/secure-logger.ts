...
export class SecureLogger {
  ...
  log(level: LogLevel, message: string, metadata?: Record<string, unknown>): void {
    ...
    let secureMessage =
      this.enableRedaction && this.redactor ? this.redactor.redact(message) : message;
  ...
  child(bindings: Record<string, unknown>): SecureLogger {
    ...
    const secureBindings =
      this.enableRedaction && this.redactor ? this.redactor.redactObject(bindings) : bindings;
 ...
 */
export function createSecureLogTransport(options: SecureLoggerOptions = {}): LogTransport {
  const redactor =
    options.enableRedaction !== false ? new CredentialRedactor(options.redactorOptions) : undefined;
