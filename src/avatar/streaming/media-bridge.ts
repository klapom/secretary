...
export class MediaBridge extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    this.log(
      "info",
      `Media bridge started (${this.config.videoFps}fps, ${this.config.videoWidth}x${this.config.videoHeight})`,
    );
    this.emit("started");
  }
  ...
  async pushVideoFrame(frame: VideoFrame): Promise<void> {
    ...
    this.log(
      "debug",
      `Video frame ${this.frameCount}: ${frame.width}x${frame.height}, latency: ${latency}ms`,
    );
  }
  ...
  async pushAudioChunk(chunk: AudioChunk): Promise<void> {
    ...
    this.log(
      "debug",
      `Audio chunk ${this.audioChunkCount}: ${chunk.data.length} bytes, ${chunk.sampleRate}Hz`,
    );
  }
 ...
 */
export function connectMediaBridge(bridge: MediaBridge, server: WebRTCStreamingServer): void {
