...
const signalingMessageSchema = z.object({
  type: z.enum(["offer", "answer", "ice-candidate", "error", "ping", "pong"]),
  ...
  async start(): Promise<void> {
    ...
    this.wss.on("connection", (ws: WebSocket, req: import("node:http").IncomingMessage) => {
      ...
      if (apiKey) {
        ...
        if (token !== apiKey) {
          this.log("warn", "WebSocket connection rejected: invalid or missing token");
          ws.close(4401, "Unauthorized");
          return;
        }
      } else {
        this.log("warn", "GATEWAY_API_KEY not set, skipping WebSocket auth (development mode)");
      }
  ...
  private handleNewConnection(ws: WebSocket): void {
    ...
    ws.on("message", (data) => {
      try {
        ...
        const message = parsed.data as SignalingMessage;
