...
const log = {
  info: (...args: unknown[]) => console.log("[avatar-example]", ...args),
  warn: (...args: unknown[]) => console.warn("[avatar-example]", ...args),
  error: (...args: unknown[]) => console.error("[avatar-example]", ...args),
};
...
export async function startAvatarStreaming(): Promise<StreamingSystem> {
  log.info("Starting Avatar WebRTC Streaming System...");
  ...
  system.server.on("peer-connected", (peer) => {
    log.info(`Client connected: ${peer.id}`);
  });
  ...
  const metricsIntervalId = setInterval(() => {
    ...
    log.info(`  Avg Latency: ${Math.round(metrics.avgLatency)}ms`);
    log.info(`  Dropped Frames: ${metrics.droppedFrames}`);
  }, 5000);

  log.info("WebRTC streaming server started on port 8081");
  log.info("Open browser client: http://localhost:8080/webrtc-client.html");

  // Clean up metrics interval on shutdown
  process.on("SIGINT", () => {
...
export function integrateWithLivePortrait(system: StreamingSystem): void {
  log.info("Integrating with LivePortrait...");
  ...
  mockLivePortrait.on("frame-ready", (frame: unknown) => {
    void system.bridge.pushVideoFrame(frame as Buffer);
  });

  log.info("LivePortrait integration complete");
}
...
export function integrateWithXTTS(system: StreamingSystem): void {
  log.info("Integrating with XTTS...");
  ...
  mockXTTS.on("audio-chunk", (chunk: unknown) => {
    void system.bridge.pushAudioChunk(chunk as Buffer);
  });

  log.info("XTTS integration complete");
}
...
export function integrateWithWhisper(system: StreamingSystem): void {
  log.info("Integrating with Whisper...");

  // Simulate Whisper service
  const mockWhisper = {
    async transcribe(_audioData: Buffer): Promise<string> {
  ...
  system.bridge.on("incoming-audio", async (chunk) => {
    try {
      ...
      log.error("Whisper transcription error:", error);
    }
  });

  log.info("Whisper integration complete");
}
...
export async function main(): Promise<void> {
  try {
    ...
    log.info("  4. Watch real-time video/audio streaming!");
    ...
    log.error("Failed to start avatar streaming:", error);
    process.exit(1);
  }
}
