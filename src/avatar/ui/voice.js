...
class VoiceManager {
  ...
  async checkHealth() {
    try {
      logger.info("Checking TTS (XTTS :8082) and STT (Whisper :8083) services...");
      this.updateStatus("pending", "Connecting...");

      const [ttsResp, sttResp] = await Promise.all([
        fetch(`${CONFIG.TTS_URL}/health`, { signal: AbortSignal.timeout(5000) }),
        fetch(`${CONFIG.STT_URL}/health`, { signal: AbortSignal.timeout(5000) }),
      ]);

      if (!ttsResp.ok) {
        throw new Error(`TTS HTTP ${ttsResp.status}`);
      }
      if (!sttResp.ok) {
        throw new Error(`STT HTTP ${sttResp.status}`);
      }

      const [ttsHealth, sttHealth] = await Promise.all([ttsResp.json(), sttResp.json()]);

      logger.success("TTS service healthy", ttsHealth);
      logger.success("STT service healthy", sttHealth);
      this.updateStatus("connected", "Connected");
    } catch (error) {
      logger.error("Voice service unavailable", error.message);
  ...
  async transcribeAudio(language) {
    try {
      ...
      // Call Whisper Large V3 API (port 8083)
